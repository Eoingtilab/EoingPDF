...
def pages_from_text(text, count):
    ...
    try:
        ...
        raise ValueError('페이지는 1-3, 5, 8-6처럼 입력해 주세요.') from None
    if not result or any(p < 0 or p >= count for p in result):
        raise ValueError(f'페이지는 1부터 {count} 사이로 입력해 주세요.')
    return result
...
def open_pdf(path):
    with pdf.open(path) as doc:
        ...
        if doc.needs_pass:
            raise ValueError('암호가 걸린 PDF입니다. 암호를 해제한 사본을 선택해 주세요.')
...
def run(request, progress=lambda value, message: None, cancelled=lambda: False):
    ...
    if not paths:
        raise ValueError('파일을 먼저 추가해 주세요.')
    ...
    def tick(index, total, message):
        if cancelled():
            raise Cancelled('작업을 취소했습니다. 원본은 그대로입니다.')
        progress(int(index / max(total, 1) * 95), message)

    try:
        tick(0, 1, '문서를 준비하고 있습니다')
        ...
        for index in range(10000):
            ...
            raise ValueError('같은 이름의 파일이 너무 많습니다. 다른 폴더를 선택해 주세요.')
        progress(100, '완료했습니다')
        return target
    finally:
