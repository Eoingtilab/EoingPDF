    ...
    with open_pdf(path) as doc:
        ...
        for index, page in enumerate(doc):
            ...
            if len(sentences) > 50000:
                raise ValueError('요약할 문장이 너무 많습니다. 문서를 나눠 주세요.')
    if not sentences:
        raise ValueError('요약할 문장이 부족합니다. 글자가 선명한 문서를 선택해 주세요.')
    frequency = Counter(w for _, _, words in sentences for w in set(words))
    ranked = sorted(range(len(sentences)), key=lambda i: sum(math.log1p(frequency[w]) for w in set(sentences[i][2])) / math.sqrt(len(sentences[i][2])), reverse=True)
    selected, seen = [], set()
    for index in ranked:
        ...
        if len(selected) == limit:
            break
    lines = [f'전체 {count}페이지 · 핵심문장 {len(selected)}개', '문장 빈도 기반 발췌입니다. 생성형 AI 요약이 아니며 원문 맥락을 확인해 주세요.', '']
    if ocr_pages:
        lines.append('OCR 사용 페이지: ' + ', '.join(map(str, sorted(ocr_pages))) + '. 인식 오류가 있을 수 있으니 원문을 확인해 주세요.')
